...
public:
	void init(char * file) {
		...
		while (fin >> ip_addr) {
			...
			cout<<"roundrobin has"<<ip_addr<<endl;
		}
	}
...
public:
	...
	string getIp(string client_ip) {    //shortest path tree
...
int main (int argc, char *argv[]) {
	...
	while (true) {
		...
		parseQmsg(qmsg, &header, &question);
		...
		if (strcmp(question.QNAME, "video.cs.jhu.edu") != 0) {
			header.RCODE = 3;
			header.ANCOUNT = 0;
			int i = 0;
			...
			if (response_ip.empty()) {
				...
				convertToRmsg(rmsg, &header, &record);
			}
		}
		int len = MAX_RLEN;
		...
		cout << client_ip << '\t' <<"q\t"<< question.QNAME << '\t' <<"r\t"<< response_ip << endl;
	}
